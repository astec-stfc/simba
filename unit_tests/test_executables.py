...
def test_linux_only_codes_raise_on_windows(tmp_path):
    ...
    # codes with a real Windows build still resolve
    assert ex["elegant"] == [f"{tmp_path}/Elegant/elegant.exe"]
...
def _run_elegant_on_windows(tmp_path, simcodes_location):
    ...
    stub = SimpleNamespace(
        ...
        code="elegant",
        objectname="test",
        executables={"elegant": ["mpiexec.exe", "-np", "4", "C:/sc/Elegant/Pelegant.exe"]},
        global_parameters={
