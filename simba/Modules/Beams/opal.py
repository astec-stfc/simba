...
def read_opal_beam_file(self, filename, step=0):
    ...
    try:
        ...
        mass = (
            beamdata.attrs["TotalCharge"]
            / beamdata.attrs["TotalMass"]
            / 1e6
        ...
        if np.isclose(constants.m_e, mass, atol=1e-28):
            ...
            warn(
                "Could not determine if particle is electron or proton; setting electron mass."
            )
            mass = constants.m_e
    ...
    try:
        ...
        self._beam.t = UnitValue(
            t0 - beamdata["z"][()] / constants.speed_of_light, units="s"
        )
...
def write_opal_beam_file(self, filename, subz=0, emitted=False):
    ...
    data = np.concatenate([np.array([[str(len(x)), "", "", "", "", ""]]), beamdata])
