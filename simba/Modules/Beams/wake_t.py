...
def particle_bunch_to_beam(self, bunch, zpos=0):
    ...
    self._beam.t = UnitValue(
        self._beam.z.val / (self.Bz * constants.speed_of_light), "s"
    )
    self._beam.px = UnitValue(
        bunch.px * self.particle_rest_energy_eV * self.q_over_c, "kg*m/s"
    )
    self._beam.py = UnitValue(
        bunch.py * self.particle_rest_energy_eV * self.q_over_c, "kg*m/s"
    )
    self._beam.pz = UnitValue(
        bunch.pz * self.particle_rest_energy_eV * self.q_over_c, "kg*m/s"
    )
...
def beam_to_particle_bunch(self, zstart=0):
    ...
    qval = (
        self._beam.charge.val
        if isinstance(self._beam.charge, UnitValue)
        else self._beam.charge
    )
    ...
    with h5py.File(filename, "r") as f:
        ...
        if charge is not None:
            self._beam.charge = UnitValue(
                np.full(len(self._beam["x"]), charge / len(self._beam["x"])), "C"
            )
            self._beam.total_charge = UnitValue(charge, "C")
        else:
            if not hasattr(self, "charge"):
                raise AttributeError(
                    "Bunch charge must be part of the beam object or provided as an argument."
                )
