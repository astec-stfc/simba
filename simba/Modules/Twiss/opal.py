...
def read_opal_twiss_files(self, filename, startS=0, reset=True):
    ...
    if isinstance(filename, (list, tuple)):
        ...
        with h5py.File(filename, "r") as f:
            ...
            betax = (
                opalData["rms_x"][()] / opalData["emit_x"][()] / opalData["ref_pz"][()]
            )
            alphax = (
                (
                    -1
                    * np.sign(opalData["xpx"][()])
                    * opalData["rms_x"][()]
                    * opalData["rms_px"][()]
                )
                / opalData["emit_x"][()]
            ...
            betay = (
                opalData["rms_y"][()] / opalData["emit_y"][()] / opalData["ref_pz"][()]
            )
            alphay = (
                (
                    -1
                    * np.sign(opalData["ypy"][()])
                    * opalData["rms_y"][()]
                    * opalData["rms_py"][()]
                )
                / opalData["emit_y"][()]
            ...
            self.sigma_t.val = np.append(
                self.sigma_t.val, opalData["rms_s"][()] / constants.speed_of_light
            )
            ...
            self.sigma_p.val = np.append(
                self.sigma_p.val, np.zeros(len(opalData["rms_x"][()]))
            )
            ...
            self.gamma_x.val = np.append(self.gamma_x.val, (1 + alphax**2) / betax)
            self.gamma_y.val = np.append(self.gamma_y.val, (1 + alphay**2) / betay)
            self.beta_z.val = np.append(
                self.beta_z.val, np.zeros(len(opalData["rms_x"][()]))
            )
            self.gamma_z.val = np.append(
                self.gamma_z.val, np.zeros(len(opalData["rms_x"][()]))
            )
            self.alpha_z.val = np.append(
                self.alpha_z.val, np.zeros(len(opalData["rms_x"][()]))
            )
            ...
            self.sigma_cp.val = np.append(
                self.sigma_cp.val, np.zeros(len(opalData["rms_x"][()]))
            )
            ...
            self.element_name.val = np.append(
                self.element_name.val, np.full(len(z), lattice_name)
            )
