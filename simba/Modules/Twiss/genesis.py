...
def read_genesis_twiss_files(self, filename, startS: float = 0, reset=True):
    ...
    if isinstance(filename, (list, tuple)):
        ...
        self.enx.val = np.append(
            self.enx.val, np.full(len(s), get_mean(file["/Beam/emitx"], is_array))
        )
        self.ex.val = np.append(
            self.ex.val,
            np.full(len(s), get_mean(file["/Beam/emitx"], is_array) / gamma),
        )
        self.ecnx.val = np.append(
            self.ecnx.val, np.full(len(s), get_mean(file["/Beam/emitx"], is_array))
        )
        self.eny.val = np.append(
            self.eny.val, np.full(len(s), get_mean(file["/Beam/emitx"], is_array))
        )
        self.ey.val = np.append(
            self.ey.val,
            np.full(len(s), get_mean(file["/Beam/emitx"], is_array) / gamma),
        )
        self.ecny.val = np.append(
            self.ecny.val, np.full(len(s), get_mean(file["/Beam/emitx"], is_array))
        )
        self.beta_x.val = np.append(
            self.beta_x.val, np.full(len(s), get_mean(file["/Beam/betax"], is_array))
        )
        self.alpha_x.val = np.append(
            self.alpha_x.val, np.full(len(s), get_mean(file["/Beam/alphax"], is_array))
        )
        self.beta_y.val = np.append(
            self.beta_y.val, np.full(len(s), get_mean(file["/Beam/betay"], is_array))
        )
        self.alpha_y.val = np.append(
            self.alpha_y.val, np.full(len(s), get_mean(file["/Beam/alphax"], is_array))
        )
        self.beta_x_beam.val = np.append(
            self.beta_x_beam.val,
            np.full(len(s), get_mean(file["/Beam/betax"], is_array)),
        )
        self.beta_y_beam.val = np.append(
            self.beta_y_beam.val,
            np.full(len(s), get_mean(file["/Beam/betay"], is_array)),
        )
        self.alpha_x_beam.val = np.append(
            self.beta_x_beam.val,
            np.full(len(s), get_mean(file["/Beam/alphax"], is_array)),
        )
        self.alpha_y_beam.val = np.append(
            self.beta_y_beam.val,
            np.full(len(s), get_mean(file["/Beam/alphay"], is_array)),
        )
        self.sigma_x.val = np.append(
            self.sigma_x.val, get_mean(file["/Beam/xsize"][()], is_array)
        )
        self.sigma_y.val = np.append(
            self.sigma_y.val, get_mean(file["/Beam/ysize"][()], is_array)
        )
        ...
        self.mean_x.val = np.append(
            self.mean_x.val, get_mean(file["/Beam/xposition"][()], is_array)
        )
        self.mean_y.val = np.append(
            self.mean_y.val, get_mean(file["/Beam/yposition"][()], is_array)
        )
        if len(self.sigma_t.val) > 0:
            self.sigma_t.val = np.append(
                self.sigma_t.val, np.full(len(s), self.sigma_t.val[-1])
            )
        else:
        ...
        self.sigma_p.val = np.append(
            self.sigma_p.val,
            get_mean(file["/Beam/energyspread"][()], is_array)
            / get_mean(file["/Beam/energy"][()], is_array),
        )
        ...
        self.gamma_x.val = np.append(
            self.gamma_x.val,
            (1 + np.full(len(s), get_mean(file["/Beam/alphax"], is_array)) ** 2)
            / np.full(len(s), get_mean(file["/Beam/betax"], is_array)),
        )
        self.gamma_y.val = np.append(
            self.gamma_x.val,
            (1 + np.full(len(s), get_mean(file["/Beam/alphay"], is_array)) ** 2)
            / np.full(len(s), get_mean(file["/Beam/betay"], is_array)),
        )
        ...
        self.sigma_cp.val = np.append(
            self.sigma_cp.val,
            get_mean(file["/Beam/energyspread"][()], is_array) * 0.511 * 1e6,
        )
        ...
        self.element_name.val = np.append(self.element_name.val, np.full(len(s), ""))
