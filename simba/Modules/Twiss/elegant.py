...
def read_elegant_twiss_files(self, filename, startS=0, reset=True):
    ...
    if isinstance(filename, (list, tuple)):
        ...
        # the reference momentum shifted by the centroid offset -- not
        # `Cdelta * cp`, which discarded the reference momentum and kept only
        # the small, often-negative, centroid deviation.
        self.sigma_cp.val = np.append(self.sigma_cp.val, elegantData["Sdelta"] * cp)
        self.mean_cp.val = np.append(
            self.mean_cp.val, cp * (1 + elegantData["Cdelta"])
        )
