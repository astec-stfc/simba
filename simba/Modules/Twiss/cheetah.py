...
def cumtrapz(x: list | np.ndarray = [], y: list | np.ndarray = []):
    ...
    if isinstance(filename, (list, tuple)):
        ...
        with h5py.File(filename, "r") as f:
            ...
            self.gamma_x.val = np.append(
                self.gamma_x.val, (1 + file["alpha_x"][()] ** 2) / file["beta_x"][()]
            )
            ...
            self.gamma_y.val = np.append(
                self.gamma_y.val, (1 + file["alpha_y"][()] ** 2) / file["beta_y"][()]
            )
            ...
            self.sigma_t.val = np.append(
                self.sigma_t.val, file["sigma_tau"][()] / constants.speed_of_light
            )
            self.mean_x.val = np.append(self.mean_x.val, file["mu_x"][()])
            self.mean_y.val = np.append(self.mean_y.val, file["mu_y"][()])
            beta = np.sqrt(1 - (gamma**-2))
            self.t.val = np.append(
                self.t.val, file["s"][()] / (beta * constants.speed_of_light)
            )
            self.sigma_cp.val = np.append(self.sigma_cp.val, file["sigma_p"][()] * cp)
            self.sigma_p.val = np.append(self.sigma_p.val, file["sigma_p"][()])
            self.mux.val = np.append(
                self.mux.val,
                cumtrapz(x=self.z.val, y=1 / (self.sigma_x.val**2 / self.ex.val)),
            )
            self.muy.val = np.append(
                self.muy.val,
                cumtrapz(x=self.z.val, y=1 / (self.sigma_y.val**2 / self.ey.val)),
            )
            ...
            self.element_name.val = np.append(
                self.element_name.val, np.full(len(file["s"][()]), "")
            )
            self.lattice_name.val = np.append(
                self.lattice_name.val, np.full(len(file["s"][()]), lattice_name)
            )
            ...
            self.eta_x_beam.val = np.append(
                self.eta_x_beam.val, np.zeros(len(file["s"][()]))
            )
            self.eta_xp_beam.val = np.append(
                self.eta_xp_beam.val, np.zeros(len(file["s"][()]))
            )
            self.eta_y_beam.val = np.append(
                self.eta_y_beam.val, np.zeros(len(file["s"][()]))
            )
            self.eta_yp_beam.val = np.append(
                self.eta_yp_beam.val, np.zeros(len(file["s"][()]))
            )
            self.beta_x_beam.val = np.append(
                self.beta_x_beam.val, np.zeros(len(file["s"][()]))
            )
            self.beta_y_beam.val = np.append(
                self.beta_y_beam.val, np.zeros(len(file["s"][()]))
            )
            self.alpha_x_beam.val = np.append(
                self.alpha_x_beam.val, np.zeros(len(file["s"][()]))
            )
            self.alpha_y_beam.val = np.append(
                self.alpha_y_beam.val, np.zeros(len(file["s"][()]))
            )
            # self.cp_eV = self.cp
