...
yaml.add_representer(NoflowList, noflow_list_rep)
yaml.add_representer(FlowList, flow_list_rep)


def convert_numpy_types(v):
    if isinstance(v, (np.ndarray, list, tuple)):
        return FlowList([convert_numpy_types(l) for l in v])
...
class Converter(Framework):
    ...
    def insert_element(self, element):
        ...
        element["start"] = [
            round(elem, 7) for elem in self.element_objects[element["name"]].start
        ]
        element["end"] = [
            round(elem, 7) for elem in self.element_objects[element["name"]].end
        ]
        if "-fea-" in lname or "-feh-" in lname or "-fed-" in lname:
            element["datum"] = [
                round(elem, 7) for elem in self.element_objects[element["name"]].middle
            ]
        else:
            element["datum"] = [
                round(elem, 7) for elem in self.element_objects[element["name"]].end
