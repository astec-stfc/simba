...
class Converter(Framework):
    ...
    def load_settings(self, filename="short_240.settings"):
        ...
        if os.path.exists(filename):
            ...
            stream = open(master_lattice + filename, "r")
        self.settings = yaml.load(stream, Loader=yaml.UnsafeLoader)
        self.global_settings = self.settings["global"]
        master_run_no = (
            self.global_settings["run_no"] if "run_no" in self.global_settings else 1
        )
        self.file_settings = self.settings["files"]
    ...
    def add_element(self, name=None, type=None, **kwargs):
        ...
        self.element_objects[name] = element
    ...
    def load_elements_file(self, input):
        ...
        for f in filename:
            ...
            if os.path.isfile(f):
                ...
                with open(master_lattice + f, "r") as stream:
                    elements = yaml.load(stream, Loader=yaml.UnsafeLoader)["elements"]
            for name, elem in list(elements.items()):
                self.read_element(name, elem)
            with open(
                self.currentfile.replace("newYAML/", "testYAML/"), "w"
            ) as outfile:
                yaml.dump({"elements": self.all_data[self.currentfile]}, outfile)

    def f_slist(self, l):  # concert list into flow-style (default is block style)
    ...
    def insert_element(self, element):
        element["centre"] = [
            round(elem, 6) for elem in self.element_objects[element["name"]].middle
        ]
        ...
        if "dip" in lname:
            lname = lname + "-d"
        match = self.close_matches(self.datums, lname)
        finalmatch = None
        mdiff = 100
        if not "-feb-" in lname:
            if not match == []:
        ...
        if finalmatch is None or mdiff > 0.001:
            ...
            if "-feb-" in lname:
                element["start"] = [
                    round(elem, 6)
                    for elem in self.element_objects[element["name"]].start
                ]
                element["end"] = [
                    round(elem, 6) for elem in self.element_objects[element["name"]].end
                ]
                ...
                element["old_datum"] = [
                    round(elem, 6) for elem in element["position_end"]
                ]
        else:
            element["old_datum"] = [
                round(elem, 6) for elem in self.datums[finalmatch.lower()]
            ]

        subelem = element["subelement"]
        newelement = OrderedDict()
        [
            newelement.update({k: self.convert_numpy_types(element[k])})
        ...
        if not subelem == "":
            ...
            self.all_data[self.currentfile][subelem]["sub_elements"][
                element["name"]
            ] = element
        else:
            self.all_data[self.currentfile][element["name"]] = element

    def close_matches(self, patterns, word):
