...
Classes:
    ...
    - :class:`~simba.Codes.Genesis.Genesis.GenesisSetupCommand`: Class for defining the
    &setup portion of the Genesis input file.

    - :class:`~simba.Codes.Genesis.Genesis.GenesisAlterSetupCommand`: Class for defining the
    &alter_setup portion of the Genesis input file.

    - :class:`~simba.Codes.Genesis.Genesis.GenesisLatticeCommand`: Class for defining the
    &lattice portion of the Genesis input file.

    - :class:`~simba.Codes.Genesis.Genesis.GenesisTimeCommand`: Class for defining the
    &time portion of the Genesis input file.

    - :class:`~simba.Codes.Genesis.Genesis.GenesisProfileConstCommand`: Class for defining the
    &profile_const portion of the Genesis input file.

    - :class:`~simba.Codes.Genesis.Genesis.GenesisProfileGaussCommand`: Class for defining the
    &profile_gauss portion of the Genesis input file.

    - :class:`~simba.Codes.Genesis.Genesis.GenesisProfileStepCommand`: Class for defining the
    &profile_step portion of the Genesis input file.

    - :class:`~simba.Codes.Genesis.Genesis.GenesisProfilePolynomCommand`: Class for defining the
    &profile_polynom portion of the Genesis input file.

    - :class:`~simba.Codes.Genesis.Genesis.GenesisProfileFileCommand`: Class for defining the
    &profile_file portion of the Genesis input file.

    - :class:`~simba.Codes.Genesis.Genesis.GenesisSequenceConstCommand`: Class for defining the
    &sequence_const portion of the Genesis input file.

    - :class:`~simba.Codes.Genesis.Genesis.GenesisSequencePolynomCommand`: Class for defining the
    &sequence_polynom portion of the Genesis input file.

    - :class:`~simba.Codes.Genesis.Genesis.GenesisSequencePowerCommand`: Class for defining the
    &sequence_power portion of the Genesis input file.

    - :class:`~simba.Codes.Genesis.Genesis.GenesisSequenceRandomCommand`: Class for defining the
    &sequence_random portion of the Genesis input file.

    - :class:`~simba.Codes.Genesis.Genesis.GenesisBeamCommand`: Class for defining the
    &beam portion of the Genesis input file.

    - :class:`~simba.Codes.Genesis.Genesis.GenesisAlterBeamCommand`: Class for defining the
    &alter_beam portion of the Genesis input file.

    - :class:`~simba.Codes.Genesis.Genesis.GenesisFieldCommand`: Class for defining the
    &field portion of the Genesis input file.

    - :class:`~simba.Codes.Genesis.Genesis.GenesisImportdistributionCommand`: Class for defining the
    &importdistribution portion of the Genesis input file.

    - :class:`~simba.Codes.Genesis.Genesis.GenesisImportbeamCommand`: Class for defining the
    &importbeam portion of the Genesis input file.

    - :class:`~simba.Codes.Genesis.Genesis.GenesisImportfieldCommand`: Class for defining the
    &importfield portion of the Genesis input file.

    - :class:`~simba.Codes.Genesis.Genesis.GenesisImporttransformationCommand`: Class for defining the
    &importtransformation portion of the Genesis input file.

    - :class:`~simba.Codes.Genesis.Genesis.GenesisEfieldCommand`: Class for defining the
    &efield portion of the Genesis input file.

    - :class:`~simba.Codes.Genesis.Genesis.GenesisSponradCommand`: Class for defining the
    &sponrad portion of the Genesis input file.

    - :class:`~simba.Codes.Genesis.Genesis.GenesisWakeCommand`: Class for defining the
    &wake portion of the Genesis input file.

    - :class:`~simba.Codes.Genesis.Genesis.GenesisSortCommand`: Class for defining the
    &sort portion of the Genesis input file.

    - :class:`~simba.Codes.Genesis.Genesis.GenesisWriteCommand`: Class for defining the
    &write portion of the Genesis input file.

    - :class:`~simba.Codes.Genesis.Genesis.GenesisTrackCommand`: Class for defining the
...
class GenesisLattice(FrameworkLattice):
    ...
    """If True, use `zmatch` in :class:`~simba.Codes.Genesis.Genesis.GenesisLatticeCommand`
    and set the location to the middle of the first undulator"""

    field_power: float = 1e3
    """Initial power for :class:`~simba.Codes.Genesis.Genesis.GenesisFieldCommand`"""

    dgrid: float = 1e-4
    """Grid size for :class:`~simba.Codes.Genesis.Genesis.GenesisFieldCommand`"""

    ngrid: int = 251
    """Number of grids for :class:`~simba.Codes.Genesis.Genesis.GenesisFieldCommand`"""

    waist_size: float = 1e-5
    """Waist size for :class:`~simba.Codes.Genesis.Genesis.GenesisFieldCommand`"""
    ...
    def write(self) -> None:
        ...
        :func:`~simba.Codes.Genesis.Genesis.write_elements` and
        based on the output of :func:`~simba.Codes.Genesis.Genesis.create_command_files`.
        """
        lattice_file = (
            ...
            for cfileid in command_files_order:
                if cfileid in self.command_files:
                    ...
                    if isinstance(cfile, GenesisCommandFile):
                        save_file(command_file, cfile.write_genesis(), "a")
                    elif isinstance(cfile, list):
                        for cf in cfile:
                            save_file(command_file, cf.write_genesis(), "a")
    ...
    def pre_process(self) -> None:
        ...
        if self.match_location:
            self.command_files["lattice"] = GenesisLatticeCommand(
                zmatch=self.match_location,
            )
        self.command_files["field"] = GenesisFieldCommand(
            power=self.field_power,
            ngrid=self.ngrid,
            dgrid=self.dgrid,
            waist_size=self.waist_size,
            waist_pos=self.wigglers[0].physical.middle.z - self.start_object.physical.start.z
        )
        self.command_files["track"] = GenesisTrackCommand()
        self.command_files["write"] = GenesisWriteCommand(
    ...
    def post_process(self) -> None:
        ...
        :attr:`~simba.Codes.Genesis.Genesis.command_files` is also cleared
        """
        super().post_process()
        beam = rbf.Beam()
    ...
    def write_setup_file(self) -> None:
        ...
        self.command_files["setup"] = GenesisSetupCommand(
    ...
    def write_time(self) -> None:
        ...
        self.command_files["time"] = GenesisTimeCommand(
            slen=slen,
            time=True,
            sample=1
        )
    ...
    def hdf5_to_genesis(self) -> None:
        ...
        if self.beam_type == "profile":
            ...
            props = {}
            self.command_files["profile_file"] = []
            for b in beam_profile_properties:
                self.command_files["profile_file"].append(
                    GenesisProfileFileCommand(
                        label=f"{b}_profile",
            ...
            self.command_files["beam"] = GenesisBeamCommand(**props)
            ...
            self.command_files["beam"] = GenesisBeamCommand(**beam_properties)
        else:
...
class GenesisSetupCommand(GenesisCommandFile):
    """
...
class GenesisAlterSetupCommand(GenesisCommandFile):
    """
    ...
    :class:`~simba.Codes.Genesis.Genesis.GenesisWriteCommand`)"""
...
class GenesisLatticeCommand(GenesisCommandFile):
    """
...
class GenesisTimeCommand(GenesisCommandFile):
    """
    ...
    :class:`~simba.Codes.Genesis.Genesis.GenesisSetupCommand` namelist, 
...
class GenesisProfileConstCommand(GenesisCommandFile):
    """
...
class GenesisProfileGaussCommand(GenesisCommandFile):
    """
...
class GenesisProfileStepCommand(GenesisCommandFile):
    """
...
class GenesisProfilePolynomCommand(GenesisCommandFile):
    """
...
class GenesisProfileFileCommand(GenesisCommandFile):
    """
...
class GenesisSequenceConstCommand(GenesisCommandFile):
    """
...
class GenesisSequencePolynomCommand(GenesisCommandFile):
    """
...
class GenesisSequencePowerCommand(GenesisCommandFile):
    """
...
class GenesisSequenceRandomCommand(GenesisCommandFile):
    """
...
class GenesisBeamCommand(GenesisCommandFile):
    """
...
class GenesisAlterBeamCommand(GenesisCommandFile):
    """
...
class GenesisFieldCommand(GenesisCommandFile):
    """
...
class GenesisImportdistributionCommand(GenesisCommandFile):
    """
...
class GenesisImportbeamCommand(GenesisCommandFile):
    """
...
class GenesisImportfieldCommand(GenesisCommandFile):
    """
...
class GenesisImporttransformationCommand(GenesisCommandFile):
    """
...
class GenesisEfieldCommand(GenesisCommandFile):
    """
...
class GenesisSponradCommand(GenesisCommandFile):
    """
...
class GenesisWakeCommand(GenesisCommandFile):
    """
...
class GenesisSortCommand(GenesisCommandFile):
    """
...
class GenesisWriteCommand(GenesisCommandFile):
    """
...
class GenesisTrackCommand(GenesisCommandFile):
    """
    ...
    :class:`~simba.Codes.Genesis.Genesis.GenesisTimeCommand`."""
...
__getattr__ = deprecated_aliases(
    ...
    {
        "commandFilesOrder": "command_files_order",
        "genesisCommandFile": "GenesisCommandFile",
        "genesisLattice": "GenesisLattice",
        "genesis_alter_beam_command": "GenesisAlterBeamCommand",
        "genesis_alter_setup_command": "GenesisAlterSetupCommand",
        "genesis_beam_command": "GenesisBeamCommand",
        "genesis_efield_command": "GenesisEfieldCommand",
        "genesis_field_command": "GenesisFieldCommand",
        "genesis_importbeam_command": "GenesisImportbeamCommand",
        "genesis_importdistribution_command": "GenesisImportdistributionCommand",
        "genesis_importfield_command": "GenesisImportfieldCommand",
        "genesis_importtransformation_command": "GenesisImporttransformationCommand",
        "genesis_lattice_command": "GenesisLatticeCommand",
        "genesis_profile_const_command": "GenesisProfileConstCommand",
        "genesis_profile_file_command": "GenesisProfileFileCommand",
        "genesis_profile_gauss_command": "GenesisProfileGaussCommand",
        "genesis_profile_polynom_command": "GenesisProfilePolynomCommand",
        "genesis_profile_step_command": "GenesisProfileStepCommand",
        "genesis_sequence_const_command": "GenesisSequenceConstCommand",
        "genesis_sequence_polynom_command": "GenesisSequencePolynomCommand",
        "genesis_sequence_power_command": "GenesisSequencePowerCommand",
        "genesis_sequence_random_command": "GenesisSequenceRandomCommand",
        "genesis_setup_command": "GenesisSetupCommand",
        "genesis_sort_command": "GenesisSortCommand",
        "genesis_sponrad_command": "GenesisSponradCommand",
        "genesis_time_command": "GenesisTimeCommand",
        "genesis_track_command": "GenesisTrackCommand",
        "genesis_wake_command": "GenesisWakeCommand",
        "genesis_write_command": "GenesisWriteCommand",
    },
)
