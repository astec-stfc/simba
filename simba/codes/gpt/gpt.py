...
from ...modules.gdf_beam import GdfBeam
...
class GptLattice(FrameworkLattice):
    ...
    _DEPRECATED_METHOD_ALIASES = {
        "endScreenObject": "end_screen_object",
        "override_meanBz": "override_mean_bz",
    ...
    override_mean_bz: float | int | None = None
    ...
    def space_charge_mode(self) -> str | None:
        ...
        :attr:`~simba.Framework_objects.frameworkLattice.global_settings` or
        ...
        if (
            ...
            "charge" in self.global_settings
            and "space_charge_mode" in self.global_settings["charge"]
        ):
            return self.global_settings["charge"]["space_charge_mode"]
        else:
            return None
    ...
    def write_elements(self) -> str:
        ...
        fulltext = self.section.to_gpt(
            startz=self.start_object.physical.start.z,
            endz=self.end_object.physical.end.z,
    ...
    def write(self) -> None:
        """
        Writes the GPT input file from :func:`~simba.Codes.GPT.gptLattice.write_elements`
    ...
    def post_process(self) -> None:
        ...
        svals = np.array(self.get_s_values(at_entrance=False)) + self.start_object.physical.start.z
        ...
        sval = np.interp(self.end_object.physical.middle.z, zvals, svals)
        self.gdf_to_hdf5(
            gptbeamfilename=self.objectname + "_out.gdf",
            screen=self.end_object,
            cathode=cathode,
            gdf=gdfbeam,
            t0=self.headers["setfile"].time,
            sval=sval,
    ...
    def hdf5_to_gdf(self, prefix: str="") -> None:
        ...
        :attr:`~simba.Codes.GPT.GPT.gptLattice.override_mean_bz`,
        ...
        if self.override_mean_bz is not None and isinstance(
            self.override_mean_bz, (int, float)
        ):
            meanBz = self.override_mean_bz
        else:
        ...
        if self.override_tout is not None and isinstance(
            ...
            endpos = (
                    self.find_s(self.end_object.name)[0][1]
                    - self.find_s(self.start_object.name)[0][1]
            )
            self.headers["tout"] = GptTout(
                starttime=0,
        ...
        rbf.gdf.write_gdf_beam_file(
            ...
            normaliseX=self.start_object.physical.middle.x,
            cathode=cathode,
        )
