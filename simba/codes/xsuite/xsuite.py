...
class XsuiteLattice(FrameworkLattice):
    ...
    def setup_collective_effects(self) -> None:
        ...
        if "charge" in list(self.file_block.keys()):
            if (
                ...
                gridsize = self.grids.get_grid_sizes(
                ...
                xf.install_spacecharge_frozen(
                    ...
                    sigma_z=sigma_z,
                    num_spacecharge_interactions=len(self.get_names())
                )
    ...
    def write(self) -> None:
        """
        Create the lattice object via :func:`~simba.Codes.Xsuite.Xsuite.xsuiteLattice.write_elements`
    ...
    def post_process(self) -> None:
        ...
        svals = self.get_s_values(as_dict=True)
        beam.read_xsuite_beam_file(
            bfname,
            zstart=self.end_object.physical.middle.z,
            s=svals[self.end],
        ...
        if self.ref_s is None:
            self.ref_s = self.start_object.physical.start.z
        df["s"] += self.ref_s
        svals = np.array(self.get_s_values(at_entrance=False)) + df["s"][0]
