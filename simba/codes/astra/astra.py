...
from ...framework_objects import FrameworkLattice, GlobalError
...
class AstraLattice(FrameworkLattice):
    ...
    _DEPRECATED_METHOD_ALIASES = {
        "find_ASTRA_filename": "find_astra_filename",
    ...
    def model_post_init(self, __context: Any) -> None:
        ...
        self.starting_rotation = (
            [0.0, 0.0, float(-1 * self.start_object.physical.global_rotation.theta)]
        )
        self.starting_rotation = (
        ...
        if "ASTRAsettings" not in self.global_settings:
            self.global_settings["ASTRAsettings"] = {}
        newrun_settings = self.file_block["input"] | self.global_settings["ASTRAsettings"]
        settings = deepcopy(newrun_settings)
        if "twiss" in settings:
            settings.pop("twiss")
        starting_offset = [a + b for a, b in zip(self.start_object.physical.start, self.starting_offset)]
        self.section.astra_headers["newrun"] = AstraNewRun(
            starting_offset=starting_offset,
            starting_rotation=self.starting_rotation,
            global_parameters=self.global_parameters,
            input_particle_definition = self.start_object.name,
            **settings,
        )
        ...
        output_settings = self.file_block["output"] | self.global_settings["ASTRAsettings"]
        zstart = self.start_object.physical.start.z
        self.zstop = self.end_object.physical.end.z
        ...
        if "charge" not in self.global_settings:
            self.global_settings["charge"] = {}
        space_charge_dict = self.file_block["charge"] | self.global_settings["charge"]
        charge_settings = space_charge_dict | self.global_settings["ASTRAsettings"]
        ...
        if "global_errors" not in self.global_settings:
            self.global_settings["global_errors"] = {}
        if "global_errors" in self.file_block or "global_errors" in self.global_settings:
            globalerror = GlobalError(
                objectname=self.objectname + "_global_error",
                objecttype="global_error",
                global_parameters=self.global_parameters,
            )
            error_settings = self.file_block["global_errors"] | self.global_settings["global_errors"]
    ...
    def write(self) -> None:
        """
        Writes the ASTRA input file from :func:`~simba.Codes.ASTRA.ASTRA.astraLattice.write_elements`
        ...
        save_file(code_file, self.section.to_astra())
    ...
    def post_process(self) -> None:
        ...
        svals = np.array(self.get_s_values(at_entrance=False)) + self.ref_s
        zvals = [a[-1] for a in self.get_z_values()]
        for e in self.screens_and_bpms:
    ...
    def find_astra_filename(
        ...
        for i in [0, -0.001, 0.001]:
            tempfilename = (
                    lattice
                    + "."
                    + str(int(round((scr.physical.middle.z + i - self.start_object.physical.start.z) * mult))).zfill(4)
                    + "."
            ...
            tempfilenameend = (
                    lattice
                    + "."
                    + str(int(round((self.zstop + i - self.start_object.physical.start.z) * mult))).zfill(4)
                    + "."
                    + str(master_run_no).zfill(3)
            )
