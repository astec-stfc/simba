...
class CheetahLattice(FrameworkLattice):
    ...
    def write(self) -> None:
        """
        Create the lattice object via :func:`~simba.Codes.Cheetah.Cheetah.cheetahLattice.write_elements`
        and save it as a JSON file to `master_subdir`.
        """
        success = self.write_elements()
        if success:
    ...
    def pre_process(self) -> None:
        ...
        prefix = prefix if self.track_beam else prefix + self.particle_definition
    ...
    def hdf5_to_openpmd(self, prefix="", write=True) -> None:
        ...
        self.global_parameters["beam"].beam.rematch_x_plane(**self.initial_twiss["horizontal"])
        self.global_parameters["beam"].beam.rematch_y_plane(**self.initial_twiss["vertical"])
    ...
    def screen_threaded_function(self, scr: DiagnosticElement, outname: str, name: str) -> None:
        ...
        s = 0
        try:
            s = self.element_objects[name].physical.middle.z
        except KeyError:
            s = self.element_objects[name.replace('_', "-")].physical.middle.z
        # scr.tau -= self.startObject.physical.middle.z
        rbf_cheetah.interpret_cheetah_particle_beam(
            beam,
            scr,
