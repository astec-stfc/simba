...
from ...modules.sdds_file import SDDSFile
# import mpi4py
...
class OpalLattice(FrameworkLattice):
    ...
    def write(self):
        ...
        save_file(command_file, output, "w")
    ...
    def pre_process(self):
        ...
        self.headers["track"] = OpalTrack(
            ...
            ZSTOP=self.end_object.physical.end.z - self.start_object.physical.start.z,
        )
    ...
    def post_process(self):
        elems = self.get_s_values(as_dict=True)
        svals = {}
        for s in self.screens_and_bpms:
            svals.update({s.name: elems[s.name] - self.start_object.physical.start.z})
        ...
        for elem in self.screens_and_bpms:
            if elem.name in spositions:
                ...
                beam._beam.z = UnitValue(beam._beam.z.val + self.start_object.physical.middle.z, "m")
                beam._beam.t = UnitValue(beam._beam.t.val + (self.start_object.physical.middle.z / speed_of_light), "s")
        ...
        rbf.openpmd.write_openpmd_beam_file(
            beam,
            f'{self.global_parameters["master_subdir"]}/{self.end_object.name}.openpmd.hdf5',
        )
        self.command_files = {}
        opalObject = SDDSFile()
    ...
    def hdf5_to_opal(self):
        ...
        rbf.opal.write_opal_beam_file(
            ...
            subz=self.start_object.physical.start.z,
            emitted=emitted,
        )
