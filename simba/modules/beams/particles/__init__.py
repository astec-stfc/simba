...
class Particles(DeprecatedMethodAliases, BaseModel):
    """
    ...
    - :attr:`~beta_gamma` -- beam momentum as beta*gamma.
    ...
    _DEPRECATED_METHOD_ALIASES = {
        ...
        "Ez": "ez",
        "Q": "q",
        "computeCorrelations": "compute_correlations",
        "performTransformation": "perform_transformation",
        "performTransformationPeakISlice": "perform_transformation_peak_i_slice",
        "rematchXPlane": "rematch_x_plane",
        "rematchXPlanePeakISlice": "rematch_x_plane_peak_i_slice",
        "rematchYPlane": "rematch_y_plane",
        "rematchYPlanePeakISlice": "rematch_y_plane_peak_i_slice",
    }
    ...
    def perform_transformation(
        ...
        beta: :class:`~simba.Modules.units.UnitValue` or float or bool
            The beta function to transform the arrays; if `False`, use :func:`~compute_correlations`
        alpha: :class:`~simba.Modules.units.UnitValue` or float or bool
            The alpha function to transform the arrays; if `False`, use :func:`~compute_correlations`
        nEmit: :class:`~simba.Modules.units.UnitValue` or float or bool
            The emittance to transform the arrays; if `False`, use :func:`~compute_correlations`
        ...
        S11, S12, S22 = self.compute_correlations(x, xp)
    ...
    def rematch_x_plane(
        ...
        if all([beta is not None and alpha is not None and beta is not False and alpha is not False]):
            x, xp = self.perform_transformation(self.x, self.xp, beta, alpha, nEmit)
    ...
    def rematch_y_plane(
        ...
        """
        if all([beta is not None and alpha is not None and beta is not False and alpha is not False]):
            y, yp = self.perform_transformation(self.y, self.yp, beta, alpha, nEmit)
    ...
    def perform_transformation_peak_i_slice(
        self,
        ...
        S11, S12, S22 = self.compute_correlations(xslice, xpslice)
        emit = np.sqrt(S11 * S22 - S12**2)
        beta1 = S11 / emit
        alpha1 = -S12 / emit
    ...
    def rematch_x_plane_peak_i_slice(
        self,
        beta=False,
        alpha=False,
        nEmit=False,
        ...
        """
        Rematch :attr:`~x` and :attr:`~xp` with respect to the Twiss and emittance functions given,
        or their values at the peak current slice; see :func:`~perform_transformation_peak_i_slice`.
        ...
        x, xp = self.perform_transformation_peak_i_slice(
    ...
    def rematch_y_plane_peak_i_slice(
        self,
        beta=False,
        alpha=False,
        nEmit=False,
        ...
        y, yp = self.perform_transformation_peak_i_slice(
