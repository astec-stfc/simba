...
from simba.modules.beams.particles.minimum_volume_ellipse import (
    get_min_vol_ellipse,
)
import numpy as np
...
class MVE(DeprecatedMethodAliases):
    ...
    _DEPRECATED_METHOD_ALIASES = {
        ...
        "PDF": "pdf",
        "PDFI": "pdfi",
        "mvesliceAnalysis": "mveslice_analysis",
        "slice_6D_Volume": "slice_6d_volume",
        "volume6D": "volume_6d",
    }
    ...
    def volume_6d(self, x, y, t, xp, yp, cp):
        if len(x) < 10:
            return 1e6
        else:
    ...
    def mve_emittance(self, x, xp, p=None):
        (center, radii, rotation, hullP) = get_min_vol_ellipse(list(zip(x, xp)), 0.01)
    ...
    def slice_6d_volume(self):
        ...
        self.sliceProperties["6D_Volume"] = np.array(
            [self.volume_6d(*a) for a in emitbins]
        )
