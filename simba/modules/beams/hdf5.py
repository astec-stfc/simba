...
def unrotate_beam_xz(self):
    if abs(self.theta) > 0:
        self.rotate_beam_xz(-1 * self.theta, -1 * self._beam.offset)
...
def read_hdf5_beam_file(self, filename, local=False):
    ...
    with h5py.File(filename, "r") as h5file:
        ...
        self.theta = float(theta)
        if local is True:
            rotate_beam_xz(self.theta, preOffset=self.starting_position)
...
__getattr__ = deprecated_aliases(
    ...
    globals(),
    {
        ...
        "write_HDF5_summary_file": "write_hdf5_summary_file",
    },
)
