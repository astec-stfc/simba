...
def astra_tw_field_map(fielddat, start, stop, cells, p):
...
def load_elements(
    ...
    fmap = {}
    mpd = MagnetPlottingData(kinetic_energy=kinetic_energy)
    for t in types:
        fmap[t] = {}
        if sections == "All":
            elements = [lattice[e["name"]] for e in lattice.get_element_type(t)]
        else:
            elements = []
            for s in sections:
                elements += [lattice[e["name"]] for e in lattice[s].get_element_type(t)]
...
__getattr__ = deprecated_aliases(
    ...
    globals(),
    {
        "ASTRA_TW_FieldMap": "astra_tw_field_map",
        "magnet_plotting_data": "MagnetPlottingData",
    },
)
