...
class MultiPlotWidget(DeprecatedMethodAliases, QWidget):
    ...
    _DEPRECATED_METHOD_ALIASES = {
        ...
        "clearCurves": "clear_curves",
        "curveClicked": "curve_clicked",
        "highlightCurveSignal": "highlight_curve_signal",
        "highlightPlot": "highlight_plot",
        "plotParams": "plot_params",
        "reScaleLattice": "re_scale_lattice",
        "removeCurve": "remove_curve",
        "removeData": "remove_data",
        "removeShadowPen": "remove_shadow_pen",
        "setPenAlpha": "set_pen_alpha",
        "unHighlightCurveSignal": "un_highlight_curve_signal",
        "unHighlightPlot": "un_highlight_plot",
        "updateCurve": "update_curve",
        "updateCurveHighlights": "update_curve_highlights",
    }
    ...
    highlight_curve_signal = pyqtSignal(str)
    un_highlight_curve_signal = pyqtSignal(str)
    ...
    def add_lattice_plot(self, elementlist):
        ...
        for name, p in self.multiPlotWidgets.items():
            ...
            for elem in lattice.elements:
                elem.setZValue(0)
                p.addItem(elem)
            self.re_scale_lattice(p.getViewBox())
            p.getViewBox().sigYRangeChanged.connect(self.re_scale_lattice)

    def re_scale_lattice(self, p):
    ...
    def update_curve(self, x, y, name, label):
    ...
    def clear_curves(self):
        self.remove_curve(self.curves.keys())

    def curve_clicked(self, name):
        if name in self.shadowCurves:
            self.un_highlight_plot(name)
            self.un_highlight_curve_signal.emit(name)
        else:
            self.highlight_plot(name)
            self.highlight_curve_signal.emit(name)
    ...
    def highlight_plot(self, name):
        ...
        self.update_curve_highlights()

    def un_highlight_plot(self, name):
        """highlights a particular plot"""
        # print('highligher clicked! = ', name)
        if not isinstance(name, (list, tuple)):
            name = [name]
        for n in name:
            self.remove_shadow_pen(n)
        self.update_curve_highlights()

    def update_curve_highlights(self):
        for n in self.curves.keys():
            if n in self.shadowCurves or not len(self.shadowCurves) > 0:
                self.set_pen_alpha(n, 255, 3)
            else:
                self.set_pen_alpha(n, 75, 3)
    ...
    def remove_shadow_pen(self, name):
        """add/remove a shadow pen to a given plot curve"""
        if name in self.shadowCurves:
